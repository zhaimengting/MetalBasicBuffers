             ...
             constant vector_uint2 *viewportSizePointer[[buffer(MTVertexInputIndexViewportSize)]])
{
    ...
    RasterizerData out;
    //初始化输出剪辑空间的位置
    out.clipSpacePostion = vector_float4(0.0,0.0,0.0,1.0);
    //获取当前顶点
    float2 pixelSpacePostion = vertices[vertexID].position.xy;
    ////将vierportSizePointer 从verctor_uint2 转换为vector_float2 类型
    vector_float2 viewportSize = vector_float2(*viewportSizePointer);
    //每个顶点着色器的输出位置在剪辑空间中(也称为归一化设备坐标空间,NDC),剪辑空间中的(-1,-1)表示视口的左下角,而(1,1)表示视口的右上角.
    //计算和写入 XY值到我们的剪辑空间的位置.为了从像素空间中的位置转换到剪辑空间的位置,我们将像素坐标除以视口的大小的一半.
    out.clipSpacePostion.xy = pixelSpacePostion/(viewportSize/2);
